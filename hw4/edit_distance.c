...
void edit_distance(char* str1,char* str2){
    ...
    // Initializing cost and direction for the first row and first collumn.
    ...
    // The for loop below sets fills up the matrix with arrows and cost using the 
    ...
    // This for loop prints the matirx with proper formatting.
    ...
    // The minimum distance is stored in the last block of the matrix. 
    ...
    // This while loop will backtrack the problem and store the aligned pairs but in reverse order.
    while(1){
        if(i == 0 && j==0){
            break;
        }
    ...
    // The next few for loops prints the string in correct order.
    // It is printing from the back because as explained above the string is stored that way. 
    for(i=n-1;i>=0;i--){
        printf("%c ",res1[i]);
    }
    printf("\n");
...
// Function to return minimum of the 3 numbers.
int min(int i1,int i2, int i3){
    int min = i1;
    if(i2 < min){
        min = i2;
...
// Function to store input entered from the stdin.
void store_str(char* str1, char* str2){
    scanf("%s",str1);
    scanf("%s",str2);
    printf("\nfirst: %s\n",str1);
